...
class StructureSolver:
    ...
    def solve(self):
        ...
        for round, SA_procedure in enumerate(all_SA_procedures):
            ...
            for iproc,to_combine in enumerate(SA_procedure):
                p = MP.Process(target=self.SA_solver,  args=(iproc,SA,to_combine,round,n_rounds,return_dict))
                p.start()
                jobs.append(p)
            for process in jobs:
                process.join()
            for combined_SA in return_dict.values():
                new_SA.append(combined_SA)
            for iSA in range(len(new_SA)):
                print(f'\t|new SA{iSA}|={len(new_SA[iSA])}')
            SA = new_SA
    ...
    def SA_solver(self, iproc, SA, to_combine, round, n_rounds, return_dict):
            # if len(to_combine)==1:
            #     print(f'Process {iproc + 1} skipped...')
            #     return SA[to_combine]

            print(f'Process {iproc+1} started...')
            combined_SA = []
            SA_combine = self.access_list_indices(SA, to_combine)
            i_check,n_checks=  (0,1)
            for tmp_SA in enumerate(SA_combine):
                n_checks= n_checks*len(tmp_SA)
            self.nchecks_arr[iproc] = n_checks
            ...
            for icomb, comb in enumerate(product(*SA_combine)):
                ...
                # i_check += 1
                # if i_check%10==0:
                #     self.check_arr[iproc]=i_check
                #     self.progress_update(note=f'Round {round}/{n_rounds}')
            return_dict[iproc] = combined_SA
