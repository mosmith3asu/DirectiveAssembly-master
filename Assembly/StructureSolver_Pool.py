...
class StructureSolver:
    ...
    def solve(self):
        ...
        for round, SA_procedure in enumerate(all_SA_procedures):
            ...
            for iproc, to_combine in enumerate(SA_procedure):
                ...
                print(f'\t|new SA{iproc}|={len(combined_SA)}')
    ...
    def SA_solver_pool(self, comb):
        ...
        valid = self.check_WS(self.Gpad + sum(tmp_Placement), self.npad)
        if valid: combined_SA = {"WS": sum(tmp_Placement), "Blocks": tmp_Block}
    ...
    def check_bounds(self, WS, npad, padval):
        ...
        return valid
    def check_WS(self, WS, npad,inWS=None):
    ...
    def preview_solutions(self):
        ...
        preview_arrays = []
        n_preview = 49
        if np.size(self.Solutions)>n_preview:
            for isol, sol in enumerate(self.Solutions[:n_preview]):
...
if __name__ == "__main__":
    ...
    # get_structure = 5  # Coin
    # get_structure = 6  # Scatter
    get_structure = 7  # Funnel

    # Initialize Structure -----------------------
    AllStructures = Assets.Structures()
    G = AllStructures.Structure[get_structure]
    Bi = AllStructures.BlockSet[get_structure]
    Sname= AllStructures.names[get_structure]


    Blocks = Assets.Blocks(BP=Bi)
    SS = StructureSolver(G, Bi, Blocks, SturctureName=Sname)
    SS.n_pool=12
    preview_grid(G)

    # SS.invalid_spaces["I_1x1"] = True
    # SS.invalid_spaces["I_2x1"] = True
    # SS.invalid_spaces["I_3x1"] = True
    # SS.invalid_spaces["L_2x2"] = False
